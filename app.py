...
def configure_cloudinary_from_env():
    """Configure Cloudinary explicitly so Render/env paste mistakes are easier to tolerate."""
    if not cloudinary:
        return False
...
def cloudinary_ready():
    raw = normalize_cloudinary_url()
    if not (cloudinary and raw and raw.startswith("cloudinary://")):
        return False
...
def admin_gallery():
    ...
    return render_template("admin/gallery.html", items=items, cloudinary_ready=cloudinary_ready())
...
def admin_media():
    if request.method == "POST":
        ...
        return redirect(url_for("admin_media"))
    return render_template("admin/media.html", images=local_media_images(), cloudinary_ready=cloudinary_ready())
